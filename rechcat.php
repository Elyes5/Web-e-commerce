...
if(isset($_GET['nomcat'])){
    $p= $_GET['nomcat'];
    $_SESSION['categorie']=$p;  
    $sql1 = "SELECT `idcat`,`nomcat` FROM ctategorie where nomcat='".$p."' ";
    $result1 = mysqli_query($conn,$sql1);
    if( mysqli_num_rows($result1)>0){
        ...
        <h1>toutes nos categories de produits: <?php print $nomcat  ?></h1>
           <form action='rechcat2.php?nomcat=<?php echo $p ;?>'  method='GET'>
           <?php
         ...
         <tr><td><input type='radio' name='marque' value='<?php echo $row['Marque']; ?>'></td>
            ...
            <?php }} ?>
        <tr><td colspan='2'><input type="submit" value="Confirmer" class="sub"></tr></td>
        </table>
        </form>
        <?php
    }

    $sql = "SELECT `idprod`, `nomprod`, `idcat`, `prix`, `qte`, `image`, `desc` FROM `produit`  WHERE  `idcat`='".$idcat."' ";
$result = mysqli_query($conn,$sql);
